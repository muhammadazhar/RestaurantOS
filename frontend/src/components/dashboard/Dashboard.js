    ...
    <div>
      ...
      {setup && !setup.setup_complete && (
        <div style={{ background: T.accentGlow, border: `1px solid ${T.accent}55`, borderRadius: 16, padding: '16px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <button onClick={() => navigate('/setup')} style={{ background: T.accent, color: '#000', border: 'none', borderRadius: 10, padding: '10px 20px', fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: "'Syne', sans-serif", flexShrink: 0 }}>
            Continue Setup →
          </button>
        </div>
      )}
