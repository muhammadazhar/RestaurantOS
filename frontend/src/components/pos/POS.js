    ...
    <div style={{ display: 'flex', gap: 8, height: 'calc(100vh - 56px)', position: 'relative' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, minHeight: 0, display: 'grid', gridTemplateColumns: '128px minmax(0,1fr)', gap: 6, overflow: 'hidden' }}>
          ...
          {/* Menu grid */}
          <div style={{ minHeight: 0, width: '100%', overflowY: 'auto', display: 'grid', gridTemplateColumns: 'repeat(3, 170px)', gridAutoRows: 'max-content', alignItems: 'start', justifyContent: 'start', justifyItems: 'stretch', gap: 4, alignContent: 'start', paddingBottom: 10 }}>
            {filtered.map(item => {
              ...
              return (
                <div key={item.id} style={{
                  ...
                  borderRadius: 12, overflow: 'hidden', transition: 'all 0.15s', display: 'flex', flexDirection: 'column', minHeight: 170, width: 170, justifySelf: 'stretch', aspectRatio: '1 / 1',
                }}>
                  {/* Image */}
                  <div onClick={() => handleMenuItemClick(item, variants[0])} style={{ height: 88, minHeight: 88, ...S.image, overflow: 'hidden', position: 'relative', cursor: 'pointer', flexShrink: 0 }}>
                    {item.image_url ? (
                  ...
                  <div style={{ padding: 8, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', flex: 1, background: itemCartQty ? (light ? '#f8fafc' : 'rgba(251,191,36,0.05)') : (light ? '#fff' : 'rgba(15,23,42,0.92)') }}>
                    <div>
                      <div title={item.name} style={{ fontSize: 13, fontWeight: 900, color: light ? '#0f172a' : T.text, lineHeight: 1.25, marginBottom: 5, minHeight: 32, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{item.name}</div>
      ...
      {/* POS section */}
      <div style={{ width: 560, display: 'flex', flexDirection: 'column', gap: 10, overflow: 'hidden' }}>
