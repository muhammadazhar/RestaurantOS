...
function fmtDate(dateStr) {
  return new Date(apiDate(dateStr) + 'T12:00:00');
}
...
function normalizeShift(shift) {
  return { ...shift, date: apiDate(shift.date) };
}
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        ...
        <tbody>
          ...
          {shifts.map(s => {
            ...
            const today = todayString();
            const isToday = apiDate(s.date) === today;
            const now = new Date().toTimeString().slice(0, 5);
            const ended = isToday && now > (s.end_time?.slice(0, 5) || '23:59');
            return (
              <tr key={`${s.id}-${apiDate(s.date)}`} style={{ borderBottom: `1px solid ${T.border}`, background: isToday ? T.accentGlow : 'transparent' }}>
                <td style={{ padding: '10px 12px', color: T.text, fontWeight: isToday ? 700 : 400 }}>
                  ...
                  {isToday && <span style={{ marginLeft: 6, fontSize: 10, background: T.accent, color: '#fff', borderRadius: 4, padding: '1px 5px', fontWeight: 700 }}>Today</span>}
                </td>
                      ...
                      {(s.status === 'active' || s.status === 'in_process') && (
                        <>
                          {s.status === 'active' && ended && (
                            <Btn size="sm" disabled={acting} style={{ background: T.accent, color: '#fff', border: 'none' }} onClick={() => onAction('continue', s)}>Continue</Btn>
                          )}
...
function ShiftRow({ shift, acting, onStart, onContinue, onClose }) {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      const start = new Date(apiDate(shift.date) + 'T' + (shift.start_time || '00:00').slice(0, 5));
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 0', borderBottom: `1px solid ${T.border}`, flexWrap: 'wrap' }}>
      ...
      <div style={{ flex: 1, minWidth: 160 }}>
        ...
        <div style={{ fontSize: 12, color: T.textMid, marginTop: 2 }}>
          {apiDate(shift.date)} - {shift.start_time?.slice(0, 5)} to {shift.end_time?.slice(0, 5)}
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        {shift.status === 'scheduled' && (
          <Btn size="sm" disabled={acting} style={{ background: T.green, color: '#fff', border: 'none' }}
            onClick={() => onStart(shift)}>Start</Btn>
        )}
        {isActive && ended && shift.status === 'active' && (
          <Btn size="sm" disabled={acting} style={{ background: T.accent, color: '#fff', border: 'none' }}
            onClick={() => onContinue(shift)}>Continue</Btn>
        )}
        {isActive && (
          <Btn size="sm" disabled={acting} style={{ background: T.red, color: '#fff', border: 'none' }}
            onClick={() => onClose(shift)}>Close</Btn>
        )}
      </div>
    </div>
  );
  ...
  if (!todayShifts || todayShifts.length === 0) {
    return (
      <Card style={{ padding: 28, textAlign: 'center', marginBottom: 24 }}>
        <div style={{ fontSize: 13, fontWeight: 800, marginBottom: 10 }}>No shift</div>
...
export default function MyShift() {
  ...
  const load = useCallback(async (showSpinner = true) => {
    ...
    try {
      ...
      const myShifts = myRes.data.map(normalizeShift);
      const today = todayString();
      setTodayShifts(myShifts.filter(s => apiDate(s.date) === today));
      ...
      if (isManager) {
        ...
        setShifts(sr.data.map(normalizeShift));
  ...
  const visibleShifts = view === 'calendar'
    ? shifts.filter(s => apiDate(s.date).slice(0, 7) === monthStr)
    : shifts;

  const handleAction = async (action, shift) => {
    setActing(true);
    try {
      const body = { shift_date: apiDate(shift.date) };
      let updated = null;
      if (action === 'start')    { const r = await startMyShift(shift.id, body); updated = normalizeShift(r.data); toast.success('Shift started!'); }
      if (action === 'continue') { const r = await continueMyShift(shift.id, body); updated = normalizeShift({ ...shift, ...r.data, status: 'in_process' }); toast.success('Continuing in overtime'); }
      if (action === 'close')    {
        const collected = window.prompt('Enter cashier collection amount (PKR)');
        if (collected === null) return;
        const cashierCollection = Number(collected);
        if (!Number.isFinite(cashierCollection) || cashierCollection < 0) {
          toast.error('Enter a valid cashier collection amount');
          return;
        }
        await closeMyShift(shift.id, { ...body, cashier_collection: cashierCollection });
        updated = normalizeShift({ ...shift, status: 'completed', cashier_collection: cashierCollection });
        toast.success('Shift closed');
      }
      if (updated) {
        const merge = list => list.map(s => (s.id === updated.id && apiDate(s.date) === apiDate(updated.date)) ? { ...s, ...updated } : s);
        setTodayShifts(list => {
          const merged = merge(list);
          return merged.some(s => s.id === updated.id && apiDate(s.date) === apiDate(updated.date))
            ? merged
            : apiDate(updated.date) === todayString() ? [updated, ...merged] : merged;
        });
        setShifts(merge);
      }
      await load(false);
  ...
  return (
    <div style={{ maxWidth: 900 }}>
      ...
      <ShiftPanel
        ...
        acting={acting}
        onStart={shift => handleAction('start', shift)}
        onContinue={shift => handleAction('continue', shift)}
        onClose={shift => handleAction('close', shift)}
      />
